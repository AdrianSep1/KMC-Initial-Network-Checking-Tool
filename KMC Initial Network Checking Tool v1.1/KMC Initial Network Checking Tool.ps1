# KMC Initial Network Checking Tool V1.2
...
foreach ($target in $targets) {
    ...
    try {
        $nslookupOutput = nslookup $target | Out-String

        # Filter out lines containing IPv6 addresses (which include colons ":")
        $ipv4OnlyOutput = $nslookupOutput -split "`r?`n" | Where-Object { $_ -notmatch "\s+[0-9a-fA-F]*:[0-9a-fA-F:]+.*" } | Out-String

        $summary.NSLookup += "NSLookup for $target`n$ipv4OnlyOutput"
        Write-Output $ipv4OnlyOutput
    } catch {
